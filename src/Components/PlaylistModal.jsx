   ...
   return (
     <div className='fixed top-[15rem] left-[40%] right-auto w-[20rem]' style={{display :openModal }}>
        <div className="bg-slate-800 p-[2rem] flex flex-col gap-[1rem]  border-sky-500">
          <button className='self-end text-white absolute top-1 bg-slate-700 right-1 px-2 rounded-md' onClick={() => setOpenModal("none") }> X</button>
                <div className='mt-3'>
                  <input
                    type="text"
                  ...
                  {playlist.map((item) => {
                    return (
                      <div className="flex gap-[1rem] " key={item._id}>
                        {" "}
                        <button
                          onClick={() => addVideoToPlaylist(videoId, item._id)}
